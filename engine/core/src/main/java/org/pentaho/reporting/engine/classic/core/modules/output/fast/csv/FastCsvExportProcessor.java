...
public class FastCsvExportProcessor extends AbstractReportProcessor {
  ...
  private String formatValue( final Object value, final int col, final ColumnFormatter[] formatters ) {
    ...
    if ( fmt.dateFormat != null && value instanceof Date ) {
      return fmt.dateFormat.format( (Date) value );
    }
    ...
    for ( int i = 0; i < count; i++ ) {
      try {
        ...
        if ( element instanceof Section ) {
          final Section sectionValue = (Section) element;
    ...
    try {
      if ( element.getStyle() != null ) {
        ...
        if ( visibleProp instanceof Boolean ) {
          final Boolean visibleBool = (Boolean) visibleProp;
          visible = visibleBool;
        }
      }
    } catch ( Exception e ) {
  ...
  private CsvWriteConfig createCsvWriteConfig() {
    ...
    final CSVQuoter quoter = new CSVQuoter( separatorChar );
