...
public class DefaultFlowControllerTest {
  ...
  public void performQueryDataTest() throws ReportDataFactoryException {
    ...
    Mockito
      .when( performanceMonitorContext.createStopWatch( Mockito.eq( PerformanceTags.REPORT_QUERY ), Mockito.any() ) )
      .thenReturn( stopWatch );
    DataFactory dataFactory = Mockito.mock( DataFactory.class );
    String query = "query";
    Mockito.when( dataFactory.queryData( Mockito.eq( query ), (DataRow) Mockito.any() ) ).thenReturn( new EmptyTableModel() );
    ...
    try {
      controller
        .performQueryData( dataFactory, query, queryLimit, queryTimeout, parameters, designTime, sortConstraints );
