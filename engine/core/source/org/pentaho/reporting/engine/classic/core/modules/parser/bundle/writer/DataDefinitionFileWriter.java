...
public class DataDefinitionFileWriter implements BundleWriterHandler {
  ...
  public String writeReport( final WriteableDocumentBundle bundle,
    ...
    final BundleWriterState contentState =
      new BundleWriterState( state, state.getReport(), "datadefinition.xml" );// NON-NLS
    ...
    writer
      .writeTag( BundleNamespaces.DATADEFINITION, "data-definition", rootAttributes, XmlWriterSupport.OPEN );// NON-NLS
  ...
  private void writeParameterDefinitions( final BundleWriterState state,
    ...
    if ( report instanceof SubReport ) {
      ...
      for ( int i = 0; i < inputMappings.length; i++ ) {
        ...
        writer
          .writeTag( BundleNamespaces.DATADEFINITION, "input-parameter", attList, XmlWriterSupport.CLOSE );// NON-NLS
      }
      ...
      for ( int i = 0; i < exportMappings.length; i++ ) {
        ...
        attList.setAttribute( BundleNamespaces.DATADEFINITION, "alias", mapping.getAlias() );// NON-NLS
        writer
          .writeTag( BundleNamespaces.DATADEFINITION, "export-parameter", attList, XmlWriterSupport.CLOSE );// NON-NLS
      }
      writer.writeCloseTag();
      return;
    }
    ...
    if ( report instanceof MasterReport ) {
      ...
      writer
        .writeTag( BundleNamespaces.DATADEFINITION, "parameter-definition", attList, XmlWriterSupport.OPEN );// NON-NLS
  ...
  private static void writePlainParameter( final XmlWriter writer,
    ...
    paramAttrs.setAttribute( BundleNamespaces.DATADEFINITION, "mandatory",
      String.valueOf( plainParameter.isMandatory() ) );// NON-NLS
    paramAttrs
      .setAttribute( BundleNamespaces.DATADEFINITION, "type", plainParameter.getValueType().getName() );// NON-NLS
    ...
    if ( namespaces.length == 0 ) {
      writer
        .writeTag( BundleNamespaces.DATADEFINITION, "plain-parameter", paramAttrs, XmlWriterSupport.CLOSE );// NON-NLS
    } else {
      writer
        .writeTag( BundleNamespaces.DATADEFINITION, "plain-parameter", paramAttrs, XmlWriterSupport.OPEN );// NON-NLS
  ...
  private static void writeListSelectionParameter( final XmlWriter writer,
    ...
    paramAttrs.setAttribute( BundleNamespaces.DATADEFINITION, "mandatory",
      String.valueOf( parameter.isMandatory() ) );// NON-NLS
    ...
    if ( parameter instanceof DefaultListParameter ) {
      ...
      if ( StringUtils.isEmpty( defaultListParameter.getQueryName() ) == false ) {
        paramAttrs
          .setAttribute( BundleNamespaces.DATADEFINITION, "query", defaultListParameter.getQueryName() );// NON-NLS
      }
      if ( StringUtils.isEmpty( defaultListParameter.getKeyColumn() ) == false ) {
        paramAttrs
          .setAttribute( BundleNamespaces.DATADEFINITION, "key-column", defaultListParameter.getKeyColumn() );// NON-NLS
      }
      if ( StringUtils.isEmpty( defaultListParameter.getTextColumn() ) == false ) {
        paramAttrs.setAttribute( BundleNamespaces.DATADEFINITION, "value-column",
          defaultListParameter.getTextColumn() );// NON-NLS
      }
    }
    ...
    if ( namespaces.length == 0 ) {
      writer
        .writeTag( BundleNamespaces.DATADEFINITION, "list-parameter", paramAttrs, XmlWriterSupport.CLOSE );// NON-NLS
    } else {
