                               ...
                               final FastGlobalView globalView)
  {
    ...
    if (paddingDataRow != null)
    {
      ...
      else if (dataRow.prepadding)
      {
        ...
        if (logger.isDebugEnabled())
        {
          logger.debug("Pre-Padding finished; Using dataset at cursor = " + dataRow.cursor + "; padding = " + dataRow.paddingCount);
        }
      ...
      else
      {
        ...
        if (ObjectUtilities.equalArray(tempRowKey, oldRowKey) == false)
        {
          if (logger.isDebugEnabled())
          {
            logger.debug("TempRowKey=" + Arrays.asList(tempRowKey) + "; RowKey=" + Arrays.asList(oldRowKey) + " Cursor=" + dataRow.cursor);
          }
          ...
          if (dataRow.paddingDataRow.getCurrentCursorPosition() < dataRow.paddingDataRow.getCrosstabColumnCount())
          {
            ...
            if (logger.isDebugEnabled())
            {
              logger.debug("RowKey Changed - Need post-padding; Cursor = " + dataRow.cursor + "; padding = " + dataRow.paddingCount);
            }
            dataRow.paddingDataRow.activate(globalView);
            dataRow.paddingDataRow.refreshPaddedRow(globalView, dataRow.reportDataRow);
          }
          else
          {
            ...
            if (dataRow.paddingCount > 0)
            {
              if (logger.isDebugEnabled())
              {
                logger.debug("RowKey Changed, but detected need for Pre-Padding = " + dataRow.paddingCount + "; Cursor = " + dataRow.cursor);
              }
              dataRow.paddingCount -= 1;
              dataRow.paddingDataRow.activate(globalView);
              dataRow.paddingDataRow.refreshPaddedRow(globalView, dataRow.reportDataRow);
            ...
            else
            {
              if (logger.isDebugEnabled())
              {
                logger.debug("RowKey Changed; Advance; Cursor = " + dataRow.cursor + "; padding = " + dataRow.paddingCount);
              }
              dataRow.paddingDataRow.activate(globalView);
        ...
        else
        {
          ...
          if (ObjectUtilities.equalArray(tempColKey, oldColKey))
          {
            if (logger.isDebugEnabled())
            {
              logger.debug("Row- and Column-Key still the same, staying in current crosstab-position = " + dataRow.paddingCount + "; Cursor = " + dataRow.cursor);
            }
        ...
        //    refreshData(globalView, tempReportDataRow);
          ...
          else
          {
            ...
            if (dataRow.paddingCount > 0)
            {
              if (logger.isDebugEnabled())
              {
                logger.debug("RowKey same, but detected need for Padding = " + dataRow.paddingCount + "; Cursor = " + dataRow.cursor);
              }
              dataRow.paddingCount -= 1;
              dataRow.paddingDataRow.activate(globalView);
              dataRow.paddingDataRow.refreshPaddedRow(globalView, dataRow.reportDataRow);
            ...
            else
            {
              if (logger.isDebugEnabled())
              {
                logger.debug("RowKey Same; Advance; Cursor = " + dataRow.cursor + "; padding = " + dataRow.paddingCount);
              }
              dataRow.reportDataRow = tempReportDataRow;
              dataRow.paddingDataRow.activate(globalView);
              dataRow.paddingDataRow.refreshRow(globalView, tempReportDataRow);
