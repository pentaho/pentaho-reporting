...
public class SystemPropertiesPanel extends Composite
{
  ...
  public SystemPropertiesPanel(final Composite parent)
  {
    ...
    resources = ResourceBundle.getBundle(SWTPreviewModule.BUNDLE_NAME);
  ...
  /**
   * Creates and returns a Table containing all the system properties. This
   * method returns a table that is configured so that the user can sort the
   * properties by clicking on the table header.
   * 
  ...
  public Table createSystemPropertiesTable()
  {
    ...
    final Map.Entry[] entries = (Map.Entry[]) data.entrySet().toArray(
  ...
  /**
   * Copies the selected cells in the table to the clipboard, in tab-delimited
   * format.
   */
  public class CopySystemPropertiesToClipboardAction extends Action
  {
    ...
    public void run()
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        clipboardContent.append(name).append('\t').append(value).append('\n');
      }

      clipboard.setContents(new Object[] { clipboardContent.toString() },
