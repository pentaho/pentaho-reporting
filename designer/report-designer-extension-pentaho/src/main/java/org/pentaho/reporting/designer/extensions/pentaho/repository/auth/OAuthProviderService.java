...
public class OAuthProviderService {
  ...
  /**
   * Fetches the list of enabled OAuth providers from the server.
   *
   * @param serverUrl The base Pentaho server URL (e.g. {@code http://localhost:8080/pentaho})
   * @return List of enabled OAuth providers, or empty list if none available or on error
   */
  public static List<OAuthProvider> fetchProviders( String serverUrl ) {
    ...
    try {
      ...
      if ( responseCode < 200 || responseCode >= 300 ) {
        logger.warn( "Provider lookup failed with HTTP status " + responseCode );
        return Collections.emptyList();
      }
      ...
      logger.error( "Error fetching OAuth providers from " + providersUrl, e );
      return Collections.emptyList();
    } finally {
      if ( connection != null ) {
  ...
  static List<OAuthProvider> parseProviders( String json ) {
    ...
    json = json.trim();
    if ( !json.startsWith( "[" ) ) {
      logger.warn( "Unexpected JSON response (not an array): " + json.substring( 0, Math.min( 100, json.length() ) ) );
      return providers;
    }

    // Split by "},\s*{" to separate objects
    // First strip the outer [ ]
  ...
  /**
   * Parses a single JSON object string into an OAuthProvider.
   */
  private static OAuthProvider parseProvider( String objStr ) {
  ...
  /**
   * Extracts a boolean value for a given key from a JSON object string.
   */
