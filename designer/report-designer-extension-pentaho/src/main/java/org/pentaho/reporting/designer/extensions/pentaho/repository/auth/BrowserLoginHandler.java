...
public class BrowserLoginHandler {
  ...
  public AuthenticationData startBrowserLogin( String pentahoServerUrl ) {
    try {
      ...
      // Persist the OAuth provider details so that a subsequent re-login
      // (isRetryAfterAuthFailure in LoginTask) can recover them and include
      // the authorizationUri parameter again — without this the server shows
      // the login page instead of redirecting straight to the IDP.
   ...
   * <p>
   * Both the default SSO flow and the OAuth-provider flow go through the
   * server's {@code /plugin/browser-auth/api/login} endpoint.  That endpoint
   * accepts:
   * <ul>
   *   <li>{@code callback} — the local PRD HTTP server URL that receives
   *       {@code jsessionid} and {@code username} after authentication.</li>
   *   <li>{@code authorizationUri} — the server-relative OAuth2 authorization
   *       path (e.g.&nbsp;{@code oauth2/authorization/azure}).  When present
   *       the servlet redirects the browser straight to the IDP, skipping the
   *       Pentaho login page entirely.</li>
   * </ul>
   * This matches the same contract used by PDI's
   * {@code BrowserAuthenticationService.buildAuthenticationUrl()}, so the
   * server-side {@code CallBackRedirectServlet} handles both clients
   * identically.
   */
  ...
  private String buildAuthUrl( String pentahoServerUrl, String callbackUrl ) {
    ...
    StringBuilder authUrl = new StringBuilder( pentahoServerUrl );
    authUrl.append( "/plugin/browser-auth/api/login?callback=" );
    ...
    if ( oauthProvider != null && oauthProvider.getAuthorizationUri() != null
      ...
      // Use the authorizationUri directly from the provider (e.g. "oauth2/authorization/azure")
      // — same contract as PDI's BrowserAuthenticationService.buildAuthenticationUrl()
      authUrl.append( '&' );
